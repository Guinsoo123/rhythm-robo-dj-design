...
> **Notion 同步说明**：块级公式用 `latex` 代码围栏；行内符号用 `$c_t$`、`$\pi$` 等形式。同步后均为 Notion Equation（KaTeX），见 [Notion同步说明](../04-同步与交付/Notion同步说明.md)。
...
- $\mathcal{C}$：安全约束（关节限位、力矩、姿态、接触冲击等），通过惩罚与硬裁剪共同实现。
...
腿足/四足机器人在仿真中常用广义坐标 $q$（含浮动基座位置姿态与关节角）。速度为 $\dot{q}$。机身姿态可用旋转矩阵 $R$ 或四元数表示。
...
其中 $g_0$ 为世界系重力（例如 $[0,0,-9.81]$ 方向归一化）。站立良好时，$g_{\mathrm{proj}}$ 接近机身 z 轴负方向；roll/pitch 变大时，$g_{\mathrm{proj}}$ 偏离，策略可据此恢复平衡。
...
beat tracker 输出 beat 时刻序列 ${t_i^b}$。相邻 beat 间隔理想为 $T_{\mathrm{beat}}$，实际歌曲有抖动，故用相位比单一“是否拍点”更鲁棒。
...
小节后相位 $\bar{\phi}$ 同理（downbeat 用于重拍强调）。
...
段落强度 $I_{\mathrm{sec}}$：主歌/副歌/Break 的粗粒度标签或能量归一化曲线，使副歌动作更猛。
...
$\rho_{\mathrm{tempo}}$ 为节拍跟踪置信度；低置信时部署应减小 $\alpha_{\mathrm{scale}}$。
...
- $q_{\mathrm{default}}$：稳定站姿，保证 $a \approx 0$ 时机器人仍站立。
...
$f_\theta$ 为 MLP；输入维数固定，便于调试。后续可将最近 $K$ 帧 $c_{t-K:t}$ 经小型 Temporal Encoder 再接入。
...
- $\mathcal{S}$：观测空间（实际实现为 $s_t$ 向量）。
- $\mathcal{A}$：残差动作空间。
- $P(s' \mid s,a)$：MuJoCo 一步推进；训练时对质量、摩擦、延迟等随机化。
- $R$：标量奖励，分解见下节。
- $\gamma \in (0,1]$：折扣因子。
...
机身 roll/pitch（或 $g_{\mathrm{proj}}$ 偏差）：
...
| Energy correlation | $\mathrm{corr}(E_{\mathrm{obs}}, e_{\mathrm{rms}})$ |
...
- [ ] 能从音频得到 $\phi, e_{\mathrm{rms}}, o$ 并可视化
- [ ] MuJoCo 中 $q_{\mathrm{default}}$ 可稳定站立
- [ ] $s_t$ 维数固定，含 $g_{\mathrm{proj}}, c_t, a_{t-1}$
- [ ] 奖励分项可单独 ablation
- [ ] PPO 训练 fall rate 下降
